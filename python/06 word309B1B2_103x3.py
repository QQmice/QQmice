...
# pyinstaller -F '.\06 word309B1B2_103x3.py'
import random
import time
...
userTestPos = input('請輸入測驗起點(1~3) = ? ')
userTestTimes = 103  # 每次測驗題數,餘數則會不足
wordListLen = len(wordList)
if not userTestPos.isdigit():  # if not , str.isdigit()判斷是否是數字
    print('\n輸入起點非正整數\n')
elif (int(userTestPos) < 1) or (int(userTestPos) > wordListLen//userTestTimes):
    print('\n輸入起點超過範圍' + '\n')
else:
